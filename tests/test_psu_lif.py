...
def test_contract_shapes_and_run():
    ...
    rngs = nnx.Rngs(0)
    hidden_shape = (10,)
    model = nn.PSU_LIF(hidden_shape, rngs=rngs)
...
def test_parallel_equals_sequential_scalar_beta():
    ...
    rngs = nnx.Rngs(2)
    model = nn.PSU_LIF((8,), beta=0.9, rngs=rngs)
...
def test_parallel_membrane_matches_recurrence():
    ...
    rngs = nnx.Rngs(4)
    model = nn.PSU_LIF((5,), beta=0.75, rngs=rngs)
